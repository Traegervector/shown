 ...
 * @memberof Graph
 */
export var getUserWithPhoto = async (
  graph: IGraph,
  userId?: string,
  requestedProps?: string[]
): Promise<IDynamicPerson> => {
  var anyUserValidScopes = [
    'User.ReadBasic.All',
    'User.Read.All',
    'Directory.Read.All',
    'User.ReadWrite.All',
  ...
  var currentUserValidScopes = ['User.Read', 'User.ReadWrite', ...anyUserValidScopes];

  var requiredUserScopes = userId ? anyUserValidScopes : currentUserValidScopes;
  var requiredPhotoScopes = userId ? anyUserValidPhotoScopes : currentUserValidPhotoScopes;
  ...
  var resource = userId ? `users/${userId}` : 'me';
  var fullResource = resource + (requestedProps ? `?$select=${requestedProps.toString()}` : '');

  // attempt to get user and photo from cache if enabled
  if (getIsUsersCacheEnabled()) {
    var cache = CacheService.getCache<CacheUser>(schemas.users, schemas.users.stores.users);
    ...
    if (cachedUser && getUserInvalidationTime() > Date.now() - cachedUser.timeCached) {
      ...
      if (user !== null && requestedProps) {
        var uniqueProps = requestedProps.filter(prop => !Object.keys(user).includes(prop));
  ...
  if (getIsPhotosCacheEnabled()) {
    ...
    if (cachedPhoto && getPhotoInvalidationTime() > Date.now() - cachedPhoto.timeCached) {
      ...
      try {
        var response: Photo = (await graph.api(`${resource}/photo`).get()) as Photo;
  ...
  if (!cachedPhoto && !cachedUser) {
    ...
    var response = await batch.executeAll();

    var photoResponse = response.get('photo');
    if (photoResponse) {
    ...
    var userResponse = response.get('user');
    if (userResponse) {
    ...
    try {
      ...
      var response = await getPhotoForResource(graph, resource, requiredPhotoScopes);
      if (response) {
        if (getIsPhotosCacheEnabled()) {
    ...
    // get user from graph
    try {
      var response: IDynamicPerson = (await graph
        .api(fullResource)
