...
var getClientId = () => {
  var urlParams = new window.URL(window.location.href).searchParams;
  var customClientId = urlParams.get('clientId');
...
var mockProvider = new MockProvider(true);
var msal2Provider = new Msal2Provider({
...
var SignInPanel = () => {
  var [state, setState] = useState(Providers.globalProvider.state);
...
function addUsefulLinks() {
  var linkStyle = 'color: black; text-decoration: none; text-align: center; cursor: pointer; padding-right: 0.5rem;';
  var textStyle = 'margin-left: 0.25rem; font-size: 0.75rem;';
  var linkContentStyle = 'display: flex;';
  ...
  var sidebarNode = sidebarHeader[0].parentNode;
  ...
  var npmSvgElem = createSvg(npmSvgPath);

  var npmText = document.createElement('span');
  npmText.style = textStyle;
  npmText.innerText = 'npm';

  var npmLinkContent = document.createElement('span');
  ...
  // links container
  var usefulLinksContainer = document.createElement('div');
  ...
  var sidebarSubheading = document.getElementsByClassName('sidebar-subheading');
  if (sidebarSubheading) {
    for (let i = 0; i < sidebarSubheading.length; i++) {
      var subheading = sidebarSubheading[i];
      subheading.removeAttribute('aria-expanded');
    }
  }

  function setEventOnMenuClick() {
    var expandCollapseMenu = document.getElementsByClassName('css-ulso1l');
    if (expandCollapseMenu) {
      for (let i = 0; i < expandCollapseMenu.length; i++) {
        var menu = expandCollapseMenu[i];
  ...
  function setArialLabelForExpandCollapseBtn() {
    var expandCollapseBtns = document.getElementsByClassName('css-rl1ij0');
    if (expandCollapseBtns) {
      for (let i = 0; i < expandCollapseBtns.length; i++) {
        var button = expandCollapseBtns[i];
  ...
  function btnUpDown(event) {
    var element = event.target; // button when using tabs to navigate, svg when using mouse.
    setButtonAriaLabel(element);
  }

  function setButtonAriaLabel(element) {
    var dataExpandedState = element.getAttribute('data-expanded');
    var ariaValue = dataExpandedState === 'true' ? 'expand' : 'collapse';
...
function createSvg(svgPath) {
  var boxWidth = 12;
  var boxHeight = 12;
  var imageStyle = 'height: 0.75rem; margin-top: 0.125rem;';
  ...
  var svgElemPath = document.createElementNS(xmlns, 'path');
...
function resetScreenSizingBtnTitles () {
  ...
  var screenSizingBtns = document.getElementsByClassName('css-6jc9zw');
  for (let i = 0; i < screenSizingBtns.length; i++) {
    var btn = screenSizingBtns[i];
...
var injectScripts = () => {
  ...
  var fluentUI = document.createElement('script');
...
var retry = setInterval(() => {
  ...
  var sidebar = document.querySelector('nav.sidebar-container');
  if (!sidebar) return;
